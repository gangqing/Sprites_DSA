...
class SubTensors:
    def __init__(self, config: Config):
        ...
        self.f, f_all, f_y = self.encode_f(x, "encode_f")  # [-1, 256]
        ...
        loss_f = 0
        for i in range(config.frame - 1):
            loss_f += tf.reduce_mean(tf.square(f_all[:, i, :] - f_y))
        loss_f = loss_f / (config.frame - 1)
        self.losses = [mes + loss_f]
    ...
    def encode_f(self, x, name):
        ...
        with tf.variable_scope(name):
            ...
            for i in range(self.config.frame):
                ...
                y_i = tf.concat((y_l2r, y_r2l), axis=1)
                f_all.append(y_i)
            f_all = tf.transpose(f_all, [1, 0, 2])
            y = tf.concat((y_l2r, y_r2l), axis=1)  # [-1, 1024]
            f = tf.layers.dense(y, self.config.f_size, name="dense_f")  # [-1, 256]
        return f, f_all, y
    ...
    def encode_z(self, x, f, name):
        ...
        with tf.variable_scope(name):
            ...
            z = tf.layers.dense(features, self.config.z_size, name="dense_z")  # [-1, 8, 32]
        return z
    ...
    def decode_frame(self, zf, name):
        ...
        with tf.variable_scope(name):
            ...
            base_filter = self.config.x_size // 8  # 256
            y = tf.layers.dense(zf, base_filter * final_size * final_size, activation=tf.nn.relu, name="dense_2")  # [-1, 8, 8 * 8 * 256]
